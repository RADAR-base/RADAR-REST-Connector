...
/**
 * User repository backed by the RADAR-base "rest-source-authorizer" webservice, mirroring
...
class HuaweiServiceUserRepository : HuaweiUserRepository() {
    private lateinit var userCache: CachedSet<HuaweiUser>
    ...
    override fun initialize(config: HuaweiRestSourceConnectorConfig) {
        ...
        userCache =
            CachedSet(
                ...
                makeRequest<HuaweiUsers> { url("users?source-type=Huawei") }
                    .users
                    .toHashSet()
                    .filterTo(HashSet()) { u ->
                        u.isComplete() &&
    ...
    override fun stream(): Sequence<HuaweiUser> =
        runBlocking(Dispatchers.Default) {
            val valueInCache =
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(HuaweiServiceUserRepository::class.java)
    }
}
