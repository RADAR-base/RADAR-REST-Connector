...
import org.radarbase.connect.rest.oura.user.UserNotAuthorizedException;
import org.radarbase.connect.rest.oura.user.OuraServiceUserRepository;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * Authenticator for Oura, which tries to refresh the access token if a request is unauthorized.
 */
public class TokenAuthenticator implements Authenticator {
  private static final Logger logger = LoggerFactory.getLogger(TokenAuthenticator.class);

  private final User user;
  private final OuraServiceUserRepository userRepository;

  TokenAuthenticator(User user, OuraServiceUserRepository userRepository) {
    this.user = user;
