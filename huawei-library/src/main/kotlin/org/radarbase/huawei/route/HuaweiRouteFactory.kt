...
object HuaweiRouteFactory {
    ...
    private fun snake(name: String): String =
        SNAKE_CASE_BOUNDARY.replace(name) { "${it.groupValues[1]}_${it.groupValues[2]}" }.lowercase()
    ...
    private val genericStatisticsTypes = listOf(
        Triple("continuous_body_fat_rate_statistics", "continuous.body.fat.rate.statistics", "connect_huawei_continuous_body_fat_rate_statistics"),
        Triple("continuous_body_temperature_rest_statistics", "continuous.body.temperature.rest.statistics", "connect_huawei_continuous_body_temperature_rest_statistics"),
        Triple("continuous_body_temperature_statistics", "continuous.body.temperature.statistics", "connect_huawei_continuous_body_temperature_statistics"),
        Triple("continuous_calories_bmr_statistics", "continuous.calories.bmr.statistics", "connect_huawei_continuous_calories_bmr_statistics"),
        Triple("continuous_exercise_heart_rate_statistics", "continuous.exercise_heart_rate.statistics", "connect_huawei_continuous_exercise_heart_rate_statistics"),
        ...
        Triple("continuous_skin_temperature_statistics", "continuous.skin.temperature.statistics", "connect_huawei_continuous_skin_temperature_statistics"),
        Triple("continuous_speed_statistics", "continuous.speed.statistics", "connect_huawei_continuous_speed_statistics"),
        Triple("continuous_steps_rate_statistics", "continuous.steps.rate.statistics", "connect_huawei_continuous_steps_rate_statistics"),
        ...
        Triple("instantaneous_resting_heart_rate_statistics", "instantaneous.resting_heart_rate.statistics", "connect_huawei_instantaneous_resting_heart_rate_statistics"),
    ...
    val definitions: List<HuaweiRouteDefinition> = buildList {
        add(
            HuaweiRouteDefinition("activity_record", "connect_huawei_activity_record") { repo, topic ->
                HuaweiActivityRecordRoute(repo, topic)
            },
        )

        // cgm_blood_glucose (+ .statistics variant)
        add(sampleSetDefinition("cgm_blood_glucose", "cgm_blood_glucose", "connect_huawei_cgm_blood_glucose") { f, start, _, received ->
            HuaweiCgmBloodGlucose.newBuilder().apply {
                time = start.toEpoch(); timeReceived = received.toEpoch()
                level = f.getDouble("level")
                avg = f.getInt("avg"); max = f.getInt("max"); min = f.getInt("min"); last = f.getInt("last")
            }.build()
        })
        add(sampleSetDefinition("cgm_blood_glucose_statistics", "cgm_blood_glucose.statistics", "connect_huawei_cgm_blood_glucose_statistics") { f, start, _, received ->
            HuaweiCgmBloodGlucose.newBuilder().apply {
                time = start.toEpoch(); timeReceived = received.toEpoch()
                level = f.getDouble("level")
                avg = f.getInt("avg"); max = f.getInt("max"); min = f.getInt("min"); last = f.getInt("last")
        ...
        add(sampleSetDefinition("daily_activity_summary", "daily_activity_summary", "connect_huawei_daily_activity_summary") { f, start, end, received ->
            HuaweiDailyActivitySummary.newBuilder().apply {
                time = start.toEpoch(); timeReceived = received.toEpoch(); endTime = end?.toEpoch()
        ...
        add(sampleSetDefinition("active_hours", "active_hours", "connect_huawei_active_hours") { f, start, end, received ->
            f.toActiveHours(start, end, received)
        })
        add(sampleSetDefinition("active_hours_statistics", "active_hours.statistics", "connect_huawei_active_hours_statistics") { f, start, end, received ->
            f.toActiveHours(start, end, received)
        })

        add(sampleSetDefinition("continuous_activity_fragment", "continuous.activity.fragment", "connect_huawei_continuous_activity_fragment") { f, start, end, received ->
            f.toContinuousActivityStatistics(start, end, received)
        })
        add(sampleSetDefinition("continuous_activity_statistics", "continuous.activity.statistics", "connect_huawei_continuous_activity_statistics") { f, start, end, received ->
            f.toContinuousActivityStatistics(start, end, received)
        })

        add(sampleSetDefinition("continuous_altitude_statistics", "continuous.altitude.statistics", "connect_huawei_continuous_altitude_statistics") { f, start, end, received ->
            HuaweiContinuousAltitudeStatistics.newBuilder().apply {
                time = start.toEpoch(); timeReceived = received.toEpoch(); endTime = end?.toEpoch()
                avg = f.getDouble("avg"); max = f.getDouble("max"); min = f.getDouble("min")
                ascentTotal = f.getDouble("ascent_total")
                descentTotal = f.getDouble("descent_total")
            }.build()
        })

        add(sampleSetDefinition("continuous_blood_glucose_statistics", "continuous.blood_glucose.statistics", "connect_huawei_continuous_blood_glucose_statistics") { f, start, end, received ->
        ...
        add(sampleSetDefinition("continuous_breathe_rate_statistics", "continuous.breathe_rate.statistics", "connect_huawei_continuous_breathe_rate_statistics") { f, start, end, received ->
        ...
        add(sampleSetDefinition("continuous_body_blood_pressure_statistics", "continuous.body.blood_pressure.statistics", "connect_huawei_continuous_body_blood_pressure_statistics") { f, start, end, received ->
        ...
        genericStatisticsTypes.forEach { (key, dataType, topic) ->
            add(
                sampleSetDefinition(key, dataType, topic) { f, start, end, received ->
                    HuaweiStatistics.newBuilder().apply { populateCommon(start, end, received, f) }.build()
                },
            )
        }

        add(sampleSetDefinition("continuous_calories_burnt", "continuous.calories.burnt", "connect_huawei_continuous_calories_burnt") { f, start, end, received ->
            HuaweiContinuousCaloriesBurnt.newBuilder().apply {
                time = start.toEpoch(); timeReceived = received.toEpoch(); endTime = end?.toEpoch()
                calories = f.getDouble("calories")
            }.build()
        })
        add(sampleSetDefinition("continuous_calories_consumed", "continuous.calories.consumed", "connect_huawei_continuous_calories_consumed") { f, start, end, received ->
            HuaweiContinuousCaloriesBurnt.newBuilder().apply {
                time = start.toEpoch(); timeReceived = received.toEpoch(); endTime = end?.toEpoch()
                calories = f.getDouble("calories")
            }.build()
        })
        add(sampleSetDefinition("continuous_calories_burnt_total", "continuous.calories.burnt.total", "connect_huawei_continuous_calories_burnt_total") { f, start, end, received ->
        ...
        add(sampleSetDefinition("continuous_distance_delta", "continuous.distance.delta", "connect_huawei_continuous_distance_delta") { f, start, end, received ->
            HuaweiContinuousDistanceDelta.newBuilder().apply {
                time = start.toEpoch(); timeReceived = received.toEpoch(); endTime = end?.toEpoch()
                distanceDelta = f.getDouble("distance_delta")
            }.build()
        })
        add(sampleSetDefinition("continuous_distance_total", "continuous.distance.total", "connect_huawei_continuous_distance_total") { f, start, end, received ->
        ...
        add(sampleSetDefinition("continuous_ecg_detail", "continuous.ecg_detail", "connect_huawei_continuous_ecg_detail") { f, start, end, received ->
        ...
        add(sampleSetDefinition("continuous_exercise_intensity", "continuous.exercise_intensity", "connect_huawei_continuous_exercise_intensity") { f, start, end, received ->
            f.toContinuousExerciseIntensity(start, end, received)
        })
        add(sampleSetDefinition("continuous_exercise_intensity_statistics", "continuous.exercise_intensity.statistics", "connect_huawei_continuous_exercise_intensity_statistics") { f, start, end, received ->
            f.toContinuousExerciseIntensity(start, end, received)
        })

        add(sampleSetDefinition("continuous_exercise_intensity_v2", "continuous.exercise_intensity.v2", "connect_huawei_continuous_exercise_intensity_v2") { f, start, end, received ->
            HuaweiContinuousExerciseIntensityV2.newBuilder().apply {
                time = start.toEpoch(); timeReceived = received.toEpoch(); endTime = end?.toEpoch()
                exerciseType = f.getInt("exercise_type")
            }.build()
        })
        add(sampleSetDefinition("continuous_exercise_intensity_v2_statistics", "continuous.exercise_intensity.v2.statistics", "connect_huawei_continuous_exercise_intensity_v2_statistics") { f, start, end, received ->
        ...
        add(sampleSetDefinition("continuous_sleep_fragment", "continuous.sleep.fragment", "connect_huawei_continuous_sleep_fragment") { f, start, end, received ->
        ...
        add(sampleSetDefinition("continuous_spo2_statistics", "continuous.spo2.statistics", "connect_huawei_continuous_spo2_statistics") { f, start, end, received ->
        ...
        add(sampleSetDefinition("continuous_steps_delta", "continuous.steps.delta", "connect_huawei_continuous_steps_delta") { f, start, end, received ->
            HuaweiContinuousStepsDelta.newBuilder().apply {
                time = start.toEpoch(); timeReceived = received.toEpoch(); endTime = end?.toEpoch()
                stepsDelta = f.getInt("steps_delta")
            }.build()
        })
        add(sampleSetDefinition("continuous_steps_total", "continuous.steps.total", "connect_huawei_continuous_steps_total") { f, start, end, received ->
        ...
        add(sampleSetDefinition("emotion", "emotion", "connect_huawei_emotion") { f, start, _, received ->
        ...
        add(healthRecordDefinition("health_record_dynamic_bp", "health.record.dynamic_bp", "connect_huawei_health_record_dynamic_bp") { f, start, end, received ->
            f.toHealthRecordDynamicBp(start, end, received)
        })
        add(healthRecordDefinition("health_record_bradycardia", "health.record.bradycardia", "connect_huawei_health_record_bradycardia") { f, start, end, received ->
            f.toHealthRecordHeartRateAlert(start, end, received)
        })
        add(healthRecordDefinition("health_record_tachycardia", "health.record.tachycardia", "connect_huawei_health_record_tachycardia") { f, start, end, received ->
            f.toHealthRecordHeartRateAlert(start, end, received)
        })
        add(healthRecordDefinition("health_record_hyperthermia", "health.record.hyperthermia", "connect_huawei_health_record_hyperthermia") { f, start, end, received ->
            HuaweiHealthRecordHyperthermia.newBuilder().apply {
                time = start.toEpoch(); timeReceived = received.toEpoch(); endTime = end?.toEpoch()
                highBodyTemperatureAlarm = f.getFloat("high_body_temperature_alarm")
            }.build()
        })
        add(healthRecordDefinition("health_record_low_spo2_alert", "health.record.lowSpo2Alert", "connect_huawei_health_record_low_spo2_alert") { f, start, end, received ->
        ...
        add(healthRecordDefinition("health_record_menstrual_cycle", "health.record.menstrual_cycle", "connect_huawei_health_record_menstrual_cycle") { f, start, end, received ->
        ...
        add(healthRecordDefinition("health_record_sleep", "health.record.sleep", "connect_huawei_health_record_sleep") { f, start, end, received ->
        ...
        add(sampleSetDefinition("heart_rate_variability", "heart_rate_variability", "connect_huawei_heart_rate_variability") { f, start, _, received ->
        ...
        add(sampleSetDefinition("resting_calories_statistics", "resting_calories.statistics", "connect_huawei_resting_calories_statistics") { f, start, end, received ->
        ...
        add(sampleSetDefinition("sleep_on_off_bed_record", "sleep.on_off_bed_record", "connect_huawei_sleep_on_off_bed_record") { f, start, _, received ->
        ...
        add(sampleSetDefinition("sleep_respiratory_detail", "sleep_respiratory_detail", "connect_huawei_sleep_respiratory_detail") { f, start, end, received ->
        ...
        add(sampleSetDefinition("sleep_respiratory_event", "sleep_respiratory_event", "connect_huawei_sleep_respiratory_event") { f, start, end, received ->
        ...
        add(sampleSetDefinition("vo2max", "vo2max", "connect_huawei_vo2max") { f, start, _, received ->
    ...
    private fun FieldValues.toHealthRecordDynamicBp(
        ...
        return HuaweiHealthRecordDynamicBp.newBuilder().apply {
            ...
            validCntAll = i("validCntAll"); cntAll = i("cntAll")
            maxSystolicBpAll = i("maxSystolicBpAll"); maxDiastolicBpAll = i("maxDiastolicBpAll"); maxHeartRateAll = i("maxHeartRateAll")
            midSystolicBpAll = i("midSystolicBpAll"); midDiastolicBpAll = i("midDiastolicBpAll"); midHeartRateAll = i("midHeartRateAll")
            minSystolicBpAll = i("minSystolicBpAll"); minDiastolicBpAll = i("minDiastolicBpAll"); minHeartRateAll = i("minHeartRateAll")
            avgSystolicBpAll = i("avgSystolicBpAll"); avgDiastolicBpAll = i("avgDiastolicBpAll"); avgHeartRateAll = i("avgHeartRateAll")
            stdSystolicBpAll = i("stdSystolicBpAll"); stdDiastolicBpAll = i("stdDiastolicBpAll"); stdHeartRateAll = i("stdHeartRateAll")
            coefSystolicBpAll = d("coefSystolicBpAll"); coefDiastolicBpAll = d("coefDiastolicBpAll"); coefHeartRateAll = d("coefHeartRateAll")
            loadSystolicBpAll = d("loadSystolicBpAll"); loadDiastolicBpAll = d("loadDiastolicBpAll")
            dropSystolicBpAll = d("dropSystolicBpAll"); dropDiastolicBpAll = d("dropDiastolicBpAll")
            peakSystolicBpAll = i("peakSystolicBpAll"); peakDiastolicBpAll = i("peakDiastolicBpAll")

            validCntWake = i("validCntWake"); cntWake = i("cntWake")
            maxSystolicBpWake = i("maxSystolicBpWake"); maxDiastolicBpWake = i("maxDiastolicBpWake"); maxHeartRateWake = i("maxHeartRateWake")
            midSystolicBpWake = i("midSystolicBpWake"); midDiastolicBpWake = i("midDiastolicBpWake"); midHeartRateWake = i("midHeartRateWake")
            minSystolicBpWake = i("minSystolicBpWake"); minDiastolicBpWake = i("minDiastolicBpWake"); minHeartRateWake = i("minHeartRateWake")
            avgSystolicBpWake = i("avgSystolicBpWake"); avgDiastolicBpWake = i("avgDiastolicBpWake"); avgHeartRateWake = i("avgHeartRateWake")
            stdSystolicBpWake = i("stdSystolicBpWake"); stdDiastolicBpWake = i("stdDiastolicBpWake"); stdHeartRateWake = i("stdHeartRateWake")
            coefSystolicBpWake = d("coefSystolicBpWake"); coefDiastolicBpWake = d("coefDiastolicBpWake"); coefHeartRateWake = d("coefHeartRateWake")
            loadSystolicBpWake = d("loadSystolicBpWake"); loadDiastolicBpWake = d("loadDiastolicBpWake")

            validCntSleep = i("validCntSleep"); cntSleep = i("cntSleep")
            maxSystolicBpSleep = i("maxSystolicBpSleep"); maxDiastolicBpSleep = i("maxDiastolicBpSleep"); maxHeartRateSleep = i("maxHeartRateSleep")
            midSystolicBpSleep = i("midSystolicBpSleep"); midDiastolicBpSleep = i("midDiastolicBpSleep"); midHeartRateSleep = i("midHeartRateSleep")
            minSystolicBpSleep = i("minSystolicBpSleep"); minDiastolicBpSleep = i("minDiastolicBpSleep"); minHeartRateSleep = i("minHeartRateSleep")
            avgSystolicBpSleep = i("avgSystolicBpSleep"); avgDiastolicBpSleep = i("avgDiastolicBpSleep"); avgHeartRateSleep = i("avgHeartRateSleep")
            stdSystolicBpSleep = i("stdSystolicBpSleep"); stdDiastolicBpSleep = i("stdDiastolicBpSleep"); stdHeartRateSleep = i("stdHeartRateSleep")
            coefSystolicBpSleep = d("coefSystolicBpSleep"); coefDiastolicBpSleep = d("coefDiastolicBpSleep"); coefHeartRateSleep = d("coefHeartRateSleep")
            loadSystolicBpSleep = d("loadSystolicBpSleep"); loadDiastolicBpSleep = d("loadDiastolicBpSleep")

            validCntWakeTwo = i("validCntWakeTwo"); cntWakeTwo = i("cntWakeTwo")
            maxSystolicBpWakeTwo = i("maxSystolicBpWakeTwo"); maxDiastolicBpWakeTwo = i("maxDiastolicBpWakeTwo"); maxHeartRateWakeTwo = i("maxHeartRateWakeTwo")
            midSystolicBpWakeTwo = i("midSystolicBpWakeTwo"); midDiastolicBpWakeTwo = i("midDiastolicBpWakeTwo"); midHeartRateWakeTwo = i("midHeartRateWakeTwo")
            minSystolicBpWakeTwo = i("minSystolicBpWakeTwo"); minDiastolicBpWakeTwo = i("minDiastolicBpWakeTwo"); minHeartRateWakeTwo = i("minHeartRateWakeTwo")
            avgSystolicBpWakeTwo = i("avgSystolicBpWakeTwo"); avgDiastolicBpWakeTwo = i("avgDiastolicBpWakeTwo"); avgHeartRateWakeTwo = i("avgHeartRateWakeTwo")
            stdSystolicBpWakeTwo = i("stdSystolicBpWakeTwo"); stdDiastolicBpWakeTwo = i("stdDiastolicBpWakeTwo"); stdHeartRateWakeTwo = i("stdHeartRateWakeTwo")
            coefSystolicBpWakeTwo = d("coefSystolicBpWakeTwo"); coefDiastolicBpWakeTwo = d("coefDiastolicBpWakeTwo"); coefHeartRateWakeTwo = d("coefHeartRateWakeTwo")
