...
import org.radarbase.connect.rest.dexcom.DexcomRestSourceConnectorConfig
...
@Suppress("unused")
class DexcomServiceUserRepository : DexcomUserRepository() {
    private lateinit var userCache: CachedSet<DexcomUser>
    ...
    override fun initialize(config: DexcomRestSourceConnectorConfig) {
        val containedUsers = config.dexcomUsers.toHashSet()

        client =
            createClient(
                baseUrl = URLBuilder(config.dexcomUserRepositoryUrl.toString()).build(),
                tokenUrl = config.dexcomUserRepositoryTokenUrl?.let {
                    URLBuilder(it.toString()).build()
                },
                clientId = config.dexcomUserRepositoryClientId,
                clientSecret = config.dexcomUserRepositoryClientSecret,
        ...
        userCache =
            CachedSet(
                ...
                makeRequest<DexcomUsers> { url("users?source-type=Dexcom") }
                    .users
                    .toHashSet()
                    .filterTo(HashSet()) { u ->
                        u.isComplete() &&
    ...
    override fun stream(): Sequence<DexcomUser> =
        runBlocking(Dispatchers.Default) {
            val valueInCache =
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(DexcomServiceUserRepository::class.java)
    }
}
