...
public class FitbitActivityLogAvroConverter extends FitbitAvroConverter {
    ...
    return optArray(s, "activityLevels")
        .map(levels -> {
          ...
          for (JsonNode level : levels) {
            Integer duration = optInt(level, "minutes")
                .map(t -> t * 60)
                .orElse(null);
            switch (optString(level, "name").orElse("")) {
              case "sedentary":
                activityLevels.setDurationSedentary(duration);
                break;
              case "lightly":
                activityLevels.setDurationLightly(duration);
                break;
              case "fairly":
                activityLevels.setDurationFairly(duration);
                break;
              case "very":
                activityLevels.setDurationVery(duration);
            }
          }
  ...
  private FitbitActivityHeartRate getHeartRate(JsonNode activity) {
    ...
    zones.ifPresent(z -> {
      for (JsonNode zone : z) {
        Integer minValue = optInt(zone, "min").orElse(null);
        Integer duration = optInt(zone, "minutes")
                .map(m -> m * 60)
                .orElse(null);
        switch (optString(zone, "name").orElse("")) {
          case "Out of Range":
            heartRate.setMin(minValue);
            heartRate.setDurationOutOfRange(duration);
            break;
          case "Fat Burn":
            heartRate.setMinFatBurn(minValue);
            heartRate.setDurationFatBurn(duration);
            break;
          case "Cardio":
            heartRate.setMinCardio(minValue);
            heartRate.setDurationCardio(duration);
            break;
          case "Peak":
            heartRate.setMinPeak(minValue);
            heartRate.setMax(optInt(zone, "max").orElse(null));
            heartRate.setDurationPeak(duration);
            break;
          default:
